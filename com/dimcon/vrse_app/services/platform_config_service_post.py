...
class PlatformConfigService:
    ...
    def handle_post(event, user_info, path_params):
        ...
        Inserts the new configuration record using details from the request body and Cognito claims.
        """
        try:
            body = event.get("body")
            if not body:
            ...
            # Optionally, extra Cognito attributes can be mapped to config_data
            ...
            with db_util.session_scope() as session:
                ...
                logger.info(f"Platform configuration inserted: {new_config.to_dict()}")
                # Return the inserted record details.
                return {"message": "Platform configuration inserted successfully.", "config": new_config.to_dict()}
